--[[	
	...
	/ ＲＯＣＬＯＴＨＥＳ
	Version - 0.7.11:lerp(.1) (catalog-replace + bot mod)
	...
	| [v0.7.10 mod] Catalog username now REPLACES the character's original
	| accessories (hair, hats, face, etc.) instead of stacking on top.
	| Originals are saved and restored on Reset.
	|
	| [v0.7.11 mod] New "Bot" tab: enter a Roblox username, click "Create Bot"
	| and a bot with that user's avatar + your current RoClothes attributes
	| (bundle body parts, scales, etc.) will spawn and follow you. "Clear Bots"
	| removes all spawned bots.
	
	
	
	original version 0.7
	local version 0.7.11:lerp(.1) (catalog-replace + bot mod)
...
function RoClothes(Player)
	...
	function Function.PlayerDataDefault()
		return {
			...
			OldestClothings = {
				...
				Pants = nil,
				ShirtGraphic = nil,
				Accessories = nil, -- [v0.7.10 mod] saved original accessories for restoration
			},
			CatalogTail = {},
			CatalogAccessory = {},
			CatalogRemove = {},
	...
	function Function.HumanoidDescriptionLoader(Character, HumanoidDescription, CharacterAttachment, UseBodyColor, Data, isCatalogUsername, oChar, tailCheck)
		...
		-- [v0.7.10 mod] When loading from a catalog username / outfit id, REPLACE the
		-- character's original accessories (hair, hats, face, etc.) instead of stacking
		-- the catalog user's accessories on top of them. Originals are saved so that
		-- Function.CharacterReset can restore them.
		if isCatalogUsername == true and tailCheck ~= true then
			local ReplaceTargetChar = oChar or Character
			if ReplaceTargetChar then
				-- Save originals only on the first run (when OldestClothings.Accessories is nil).
				-- On later runs the originals have already been removed, so re-saving would
				-- capture an empty set; we want to keep the very first snapshot for restore.
				if PlayerData[Data].OldestClothings.Accessories == nil then
					local saved = {}
					for _, acc in pairs(ReplaceTargetChar:GetChildren()) do
						if acc:IsA("Accessory") and not acc:HasTag("RoClothes") then
							table.insert(saved, acc:Clone())
						end
					end
					PlayerData[Data].OldestClothings.Accessories = saved
				end

				-- Remove every original accessory currently on the target character so the
				-- catalog user's accessories (added below) fully replace them.
				local toRemove = {}
				for _, acc in pairs(ReplaceTargetChar:GetChildren()) do
					if acc:IsA("Accessory") and not acc:HasTag("RoClothes") then
						table.insert(toRemove, acc)
					end
				end
				for _, acc in pairs(toRemove) do
					acc:Destroy()
				end
			end
		end
	...
	function Function.CharacterReset(PlayerName,NPCClear) -- findResetCharacter
		...
		if PartListPlayer then
			...
			-- [v0.7.10 mod] Restore the original accessories that were removed when a
			-- catalog username was applied. Works for both Method 1 (char is the real
			-- character) and Method 2 (char is the invisible clone; the real character
			-- is found via the ObjectValue that points back to the original model).
			if DataList.OldestClothings.Accessories then
				local restoreChar = char
				if restoreChar and Method == 2 then
					local charValue = restoreChar:FindFirstChildOfClass("ObjectValue")
					if charValue and charValue.Value and charValue.Value.Parent then
						restoreChar = charValue.Value
					end
				end
				if restoreChar then
					for _, acc in pairs(DataList.OldestClothings.Accessories) do
						local cloned = acc:Clone()
						cloned.Parent = restoreChar
					end
				end
				DataList.OldestClothings.Accessories = nil
			end
	...
	-- [v0.7.11 bot mod] ----------------------------------------------------------------
	-- Bots table: keyed by unique data key. Each entry is
	--   { Model, Humanoid, FollowConn, DataKey, Username }
	local Bots = {}
	...
	function Function.CreateBot(username)
		if not username or username == "" then
			warn("[RoClothes][Bot] No username provided")
			return
		end
		if not PlayerData[SelectPlayer] then
			warn("[RoClothes][Bot] SelectPlayer has no PlayerData")
			return
		end

		local ok, err = pcall(function()
			local userId = PS:GetUserIdFromNameAsync(username)
			local hd = PS:GetHumanoidDescriptionFromUserId(userId)
			local model = PS:CreateHumanoidModelFromDescription(hd, Enum.HumanoidRigType.R6)
			model.Name = "RoClothesBot_"..tostring(username)
			model.Archivable = false

			-- place the bot a few studs in front of the local player
			local lpChar = Player.Character
			if lpChar and lpChar:FindFirstChild("HumanoidRootPart") then
				model:PivotTo(lpChar.HumanoidRootPart.CFrame * CFrame.new(0, 0, -5))
			end

			model.Parent = workspace

			-- set the bot's parts to non-colliding so it doesn't push the player around
			...
			-- register in NPCs table (used by CharacterReset for character-instance lookups)
			local npcKey = "RoClothesBot_"..tostring(math.random(100000000, 999999999)).."_"..tostring(username)
			NPCs[npcKey] = model

			-- clone the SelectPlayer's PlayerData so the bot inherits all the same
			-- bundle / scale / body attributes, then reset the per-character runtime
			-- lists so they belong to the bot, not the player.
			...
			cDataTable.OldestClothings = { Shirt = nil, Pants = nil, ShirtGraphic = nil, Accessories = nil }
			cDataTable.Cooldown = false
			cDataTable.Character = model
			PlayerData[npcKey] = cDataTable

			-- apply the RoClothes attributes (R6 conversion + bundle body parts etc.)
			local ok2, err2 = pcall(Function.CharacterExecute, model, npcKey)
			if not ok2 then
				warn("[RoClothes][Bot] CharacterExecute error:", err2)
			end

			-- follow loop: walk toward a point a few studs behind the local player
			local human = model:FindFirstChildOfClass("Humanoid")
			local followConn
			followConn = RS.Heartbeat:Connect(function()
				if not model.Parent then return end
				if not model:FindFirstChildOfClass("Humanoid") then return end
				local lpChar2 = Player.Character
				if not lpChar2 or not lpChar2:FindFirstChild("HumanoidRootPart") then return end
				if not model:FindFirstChild("HumanoidRootPart") then return end
				local targetCF = lpChar2.HumanoidRootPart.CFrame
				local targetPos = (targetCF * CFrame.new(0, 0, 4)).Position
				local botPos = model.HumanoidRootPart.Position
				local dist = (targetPos - botPos).Magnitude
				if dist > 2 then
					human:MoveTo(targetPos)
				end
			end)
			table.insert(AllConnect, followConn)
			...
			if GUIObject.BotStatusText then
				GUIObject.BotStatusText.Text = "Bots: "..tostring(tableSize(Bots)).."  (last: "..tostring(username)..")"
			end
			print("[RoClothes][Bot] Created bot for:", username)
		end)
		if not ok then
			warn("[RoClothes][Bot] CreateBot error:", err)
			if GUIObject.BotStatusText then
				GUIObject.BotStatusText.Text = "Error: "..tostring(err)
			end
		end
	end

	function Function.ClearBots()
		for key, info in pairs(Bots) do
			pcall(function()
				if info.FollowConn then info.FollowConn:Disconnect() end
			end)
			pcall(function()
				Function.CharacterReset(info.Model)
			end)
			pcall(function()
			...
			pcall(function()
				if NPCs[key] then NPCs[key] = nil end
			end)
			pcall(function()
				if PlayerData[key] then PlayerData[key] = nil end
			end)
		end
		Bots = {}
		if GUIObject.BotStatusText then
			GUIObject.BotStatusText.Text = "Bots: 0  (cleared)"
		end
		print("[RoClothes][Bot] All bots cleared")
	end
	-- [/v0.7.11 bot mod] ----------------------------------------------------------------
	...
	-- [v0.7.11 bot mod] Bot tab + page setup ----------------------------------------------
	do
		local Bot_2 = Instance.new("Frame")
		Bot_2.Name = "Bot"
		Bot_2.Parent = GUIObject.ButtonFrame
		Bot_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		Bot_2.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Bot_2.Position = UDim2.new(0.784810185, 0, 0.866666734, 0)
		Bot_2.Size = UDim2.new(1, 0, 1, 0)
		local BotARC = Instance.new("UIAspectRatioConstraint", Bot_2)
		local BotCorner = Instance.new("UICorner", Bot_2)
		BotCorner.CornerRadius = UDim.new(0.25, 0)
		local BotButton = Instance.new("TextButton", Bot_2)
		BotButton.Name = "BotButton"
		BotButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		BotButton.BackgroundTransparency = 1.000
		BotButton.Size = UDim2.new(1, 0, 1, 0)
		BotButton.Font = Enum.Font.Code
		BotButton.Text = "Bot"
		BotButton.TextColor3 = Color3.fromRGB(255, 255, 255)
		BotButton.TextScaled = true
		BotButton.TextSize = 14.000
		BotButton.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
		BotButton.TextWrapped = true
		local BotGrad = Instance.new("UIGradient", Bot_2)
		...
		local BotPage = Instance.new("Frame")
		BotPage.Name = "Bot"
		BotPage.Parent = GUIObject.PageFrame
		BotPage.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		BotPage.BackgroundTransparency = 1.000
		BotPage.Size = UDim2.new(1, 0, 1, 0)
		BotPage.Visible = false

		-- Username textbox
		local usernameFrame = Instance.new("Frame")
		usernameFrame.Name = "BotUsername"
		usernameFrame.Parent = BotPage
		usernameFrame.AnchorPoint = Vector2.new(0.5, 0)
		usernameFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		usernameFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		usernameFrame.Position = UDim2.new(0.5, 0, 0.025, 0)
		usernameFrame.Size = UDim2.new(0.891309202, 0, 0.0646399707, 0)
		local usernameCorner = Instance.new("UICorner", usernameFrame)
		usernameCorner.CornerRadius = UDim.new(0.25, 0)
		local usernameTb = Instance.new("TextBox", usernameFrame)
		usernameTb.Name = "BotUsernameTextbox"
		usernameTb.BackgroundTransparency = 1.000
		usernameTb.Size = UDim2.new(1, 0, 1, 0)
		usernameTb.ClearTextOnFocus = false
		usernameTb.Font = Enum.Font.Code
		usernameTb.PlaceholderText = "Bot Username (Roblox username)"
		usernameTb.Text = ""
		usernameTb.TextColor3 = Color3.fromRGB(255, 255, 255)
		usernameTb.TextScaled = true
		usernameTb.TextSize = 14.000
		usernameTb.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
		usernameTb.TextWrapped = true
		local usernameGrad = Instance.new("UIGradient", usernameFrame)
		usernameGrad.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(22, 45, 94)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(52, 80, 145))
		}
		usernameGrad.Rotation = -90

		-- Create Bot button (green)
		local createFrame = Instance.new("Frame")
		createFrame.Name = "BotCreate"
		createFrame.Parent = BotPage
		createFrame.AnchorPoint = Vector2.new(0.5, 0)
		createFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		createFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		createFrame.Position = UDim2.new(0.5, 0, 0.11, 0)
		createFrame.Size = UDim2.new(0.891309202, 0, 0.0646399707, 0)
		local createCorner = Instance.new("UICorner", createFrame)
		createCorner.CornerRadius = UDim.new(0.25, 0)
		local createBtn = Instance.new("TextButton", createFrame)
		createBtn.Name = "BotCreateButton"
		createBtn.BackgroundTransparency = 1.000
		createBtn.Size = UDim2.new(1, 0, 1, 0)
		createBtn.Font = Enum.Font.Code
		createBtn.Text = "Create Bot"
		createBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
		createBtn.TextScaled = true
		createBtn.TextSize = 14.000
		createBtn.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
		createBtn.TextWrapped = true
		local createGrad = Instance.new("UIGradient", createFrame)
		createGrad.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(20, 90, 30)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 130, 60))
		}
		createGrad.Rotation = -90

		-- Clear Bots button (red)
		local clearFrame = Instance.new("Frame")
		clearFrame.Name = "BotClear"
		clearFrame.Parent = BotPage
		clearFrame.AnchorPoint = Vector2.new(0.5, 0)
		clearFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		clearFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		clearFrame.Position = UDim2.new(0.5, 0, 0.195, 0)
		clearFrame.Size = UDim2.new(0.891309202, 0, 0.0646399707, 0)
		local clearCorner = Instance.new("UICorner", clearFrame)
		clearCorner.CornerRadius = UDim.new(0.25, 0)
		local clearBtn = Instance.new("TextButton", clearFrame)
		clearBtn.Name = "BotClearButton"
		...
		clearBtn.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
		clearBtn.TextWrapped = true
		local clearGrad = Instance.new("UIGradient", clearFrame)
		clearGrad.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(120, 20, 20)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(180, 40, 40))
		}
		clearGrad.Rotation = -90

		-- Status label (purple)
		local statusFrame = Instance.new("Frame")
		statusFrame.Name = "BotStatusFrame"
		statusFrame.Parent = BotPage
		statusFrame.AnchorPoint = Vector2.new(0.5, 0)
		statusFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		statusFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		statusFrame.Position = UDim2.new(0.5, 0, 0.28, 0)
		statusFrame.Size = UDim2.new(0.891309202, 0, 0.0646399707, 0)
		local statusCorner = Instance.new("UICorner", statusFrame)
		statusCorner.CornerRadius = UDim.new(0.25, 0)
		local statusLabel = Instance.new("TextLabel", statusFrame)
		statusLabel.Name = "BotStatusText"
		statusLabel.BackgroundTransparency = 1.000
		statusLabel.Size = UDim2.new(1, 0, 1, 0)
		statusLabel.Font = Enum.Font.Code
		statusLabel.Text = "Bots: 0"
		statusLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
		statusLabel.TextScaled = true
		statusLabel.TextSize = 14.000
		statusLabel.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
		statusLabel.TextWrapped = true
		local statusGrad = Instance.new("UIGradient", statusFrame)
		statusGrad.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 30, 60)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(70, 50, 100))
		}
		statusGrad.Rotation = -90

		-- Expose buttons + status label + textbox to the rest of the script
		GUIObject.BotButton = BotButton
		GUIObject.BotCreateButton = createBtn
		GUIObject.BotClearButton = clearBtn
		GUIObject.BotStatusText = statusLabel
		GUIObject.BotUsernameTextbox = usernameTb
	end
	-- [/v0.7.11 bot mod] -----------------------------------------------------------------

	-- [v0.7.11 bot mod] Wire up Bot tab buttons
	do
		local botCreateConn = GUIObject.BotCreateButton.MouseButton1Click:Connect(function()
			local name = GUIObject.BotUsernameTextbox.Text
		...
		table.insert(AllConnect, botCreateConn)

		local botClearConn = GUIObject.BotClearButton.MouseButton1Click:Connect(function()
			task.spawn(function()
				Function.ClearBots()
			end)
		end)
		table.insert(AllConnect, botClearConn)
	end
	-- [/v0.7.11 bot mod] -----------------------------------------------------------------
